...
/* Clock and PLL initialization based on the RTC oscillator */
void Chip_SetupRtcClocking(uint32_t iFreq)
{
	...
	Chip_Clock_SetAsyncSysconClockSource(SYSCON_MAINCLKSRC_PLLOUT);

	//ADC clock cfg
	Chip_Clock_SetADCClockSource(SYSCON_MAINCLKSRC_PLLOUT); 
	Chip_Clock_SetADCClockDiv(2); //50m
}
