...
/* Assert a SPI select */
static void _rom_spimDEAssertSSEL(LPC_SPI_T *pSPI, uint8_t sselNum)
{
	/* Assert a SSEL line by driving it low */
	pSPI->TXCTRL |= SPI_TXDATCTL_DEASSERTNUM_SSEL(sselNum);
}
...
#else
	/* Enable SPI interrupts for master */
		pDrv->base->INTENSET =   SPI_INTENSET_TXDYEN;
#endif
...
//		rt_kprintf(1,"spim_transfer_handler[%x]\r\n",status);
