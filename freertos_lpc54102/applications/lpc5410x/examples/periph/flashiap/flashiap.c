...
/** @defgroup PERIPH_FLASHIAP_5410X FLASH read/write/signature example
 * @ingroup EXAMPLES_PERIPH_5410X
 * @include "periph\flashiap\readme.txt"
 */
...
static uint32_t src_iap_array_data1[ARRAY_ELEMENTS];
...
int main(void)
{
	...
	__enable_irq();
		...
		Rtu_Read_Data_From_Flash(START_ADDR_LAST_SECTOR,(uint8_t*)src_iap_array_data1,IAP_NUM_BYTES_TO_WRITE);
	/* Endless loop */
	while (1) {
		__WFI();
	}
