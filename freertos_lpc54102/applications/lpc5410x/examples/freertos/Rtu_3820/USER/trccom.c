...
extern xQueueHandle Sem_DEBUG_start;//Adc 数据处理信号量---10ms执行一次

TRC_COM_T	trc_com0;
TRC_COM_T	trc_com1;

TRC_COM_P trc_com_ptr0;
TRC_COM_P trc_com_ptr1;
/* Initialize ring buffer */
//uint8 vtr_accessoriestype;//选件板类型0x00:模拟常规；0x01:MPT1327；0x02:PDT；0x03:VSC；0x04:GSM；0x05:ZNCG；
#define   DEBUGOUT	rt_kprintf
void rt_kprintf(uint32 flg,const char *fmt, ...);

#define RT_CONSOLEBUF_SIZE   256*1
#define RT_SPOTOCOL_SIZE   256+128
static char rt_log_buf[RT_CONSOLEBUF_SIZE];
static char Send_Potocol_buf[RT_SPOTOCOL_SIZE];
...
//		rt_kprintf(DEBUG_SYS,"TRC_COM_init_app:Com[%d] NO SPACE ERROR--pvPortMalloc\r\n",com);
	...
	if(com==0)
	{
		tmpptr->rx_driver_buf_ptr=&rx_rb;
...
//	ret=xSemaphoreTake(Sem_DEBUG_start,5);
//	if(ret!=pdPASS)
//	{
//		return;
//	}
	...
	if(Get_Debug_Level()&(0x1<<flg))
	{
		...
		//	 rt_memset(rt_log_buf,0, sizeof(rt_log_buf));
			...
			if (length > RT_CONSOLEBUF_SIZE - 1)
				...
				bytes = RTU_Uart0_Write(rt_log_buf, length);//发送
...
void rt_pirntf_hex(uint32 flg,uint8 *buf,uint16 len)
{
	uint16 i;
...
void Send_Vtr_Up_Data(uint8 ch,uint8 *data_source,uint16 length)
{
	...
	*(Send_Potocol_buf+new_length++)=0x04;
	if(ch == 0)
		...
		RTU_Uart0_Write(Send_Potocol_buf, new_length);//发送
//			pUART=(LPC_USART_T *)Get_TRC_Com_Dev_Uart(0);
//			Chip_UART_SendRB(pUART, &Txring[0],(uint8*)Send_Potocol_buf,new_length);//发送
		}
		
		else if(ch == 1)
		{
//			pUART=(LPC_USART_T *)Get_TRC_Com_Dev_Uart(1);
//			Chip_UART_SendRB(pUART, &Txring[1],(uint8*)Send_Potocol_buf,new_length);//发送
		}
}
