...
static void spiTranSetup1(ROM_SPIM_HANDLE_T spimHandle, ROM_SPIM_XFER_T *pXfer)
{
	...
	ROM_DMA_QueueDescriptor(dmaHandle, &spiDmaTXQueue1, &dmaSPITXDesc);
...
void spiRecvSetup1(ROM_SPIM_HANDLE_T spimHandle, ROM_SPIM_XFER_T *pXfer)
{
	...
	/* Queue descriptor for SPI TX transfer */
	ROM_DMA_QueueDescriptor(dmaHandle, &spiDmaRXQueue1, &dmaSPIRXDesc);
...
void RTU_SpiDev_Init()
{
	uint32_t memSize, *devMem;
		...
		/* Setup SPI1SPI1SPI1SPI1SPI1SPI1 TX channel for peripheral request and init queue */
		...
		/* Register error, descriptor completion, and descriptor chain completion callbacks for SPI TX channel */
		ROM_DMA_RegisterQueueCallback(dmaHandle, &spiDmaTXQueue1, ROM_DMA_XFERCOMPLETE_CB, (void *) dmaSPITXTransferComplete);
		ROM_DMA_RegisterQueueCallback(dmaHandle,
									  &spiDmaTXQueue1,
									  ROM_DMA_XFERDESCCOMPLETE_CB,
									  (void *) dmaSPITXTransferDescComplete);
		ROM_DMA_RegisterQueueCallback(dmaHandle, &spiDmaTXQueue1, ROM_DMA_XFERERROR_CB, (void *) dmaSPITXTransferError);
		...
		ROM_DMA_RegisterQueueCallback(dmaHandle,
									  ...
									  ROM_DMA_XFERDESCCOMPLETE_CB,
									  (void *) dmaSPIRXTransferDescComplete);
		ROM_DMA_RegisterQueueCallback(dmaHandle, &spiDmaRXQueue1, ROM_DMA_XFERERROR_CB, (void *) dmaSPIRXTransferError);
		...
		/* SPI1111111111111111111 INITTTTTTGet needed size for SPI driver context memory */
		memSize = ROM_SPIM_GetMemSize();
