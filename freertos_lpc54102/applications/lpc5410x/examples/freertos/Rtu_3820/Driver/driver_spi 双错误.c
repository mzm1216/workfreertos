...
/* ROM driver handle for SPI master and DMA */
 ...
 ROM_SPIM_HANDLE_T spimHandle1;
 ROM_DMA_HANDLE_T dmaHandle1;

/* SPI and DMA driver context areas */
 uint32_t spiDrvData[16];
 uint32_t dmaDrvData[16];
 /* SPI and DMA driver context areas */
  uint32_t spiDrvData1[16];
  uint32_t dmaDrvData1[16];
...
ALIGN(16)  ROM_DMA_DESC_T dmaSPIRXDesc1;
...
//	Chip_IOCON_PinMuxSet(LPC_IOCON, 0, 13, (IOCON_FUNC1 | IOCON_DIGITAL_EN | IOCON_MODE_PULLUP));
	Chip_IOCON_PinMuxSet(LPC_IOCON, 1, 14, (IOCON_FUNC4 | IOCON_DIGITAL_EN | IOCON_MODE_PULLUP));
...
static void spiTranSetup1(ROM_SPIM_HANDLE_T spimHandle, ROM_SPIM_XFER_T *pXfer)
{
	...
	dmaXferCfg1.src  = (void *) tx16;
	dmaXferCfg1.dest = (void *) &LPC_SPIM_PORT1->TXDAT;

	/* Setup source to desination copy for trigger for memory */
	dmaXferCfg1.xferCount = BUFFSIZE;					/* Transfer data values of size width */
	dmaXferCfg1.swTrig = 1;									/* No software triggering */
	dmaXferCfg1.clrTrig = 0;									/* Do not clear trigger after this descriptor completes */
	dmaXferCfg1.fireDescCB = 1;							/* Do not fire descriptor callback on complettion of this descriptor */
	dmaXferCfg1.enabCirc = 0;
	dmaXferCfg1.dmaCh = spiDmaTXQueue1.dmaCh;
	dmaXferCfg1.stallDesc = 0;
	if (flen > 8) {
		dmaXferCfg1.width = ROM_DMA_WIDTH_2;			/* Width is 2 bytes */
	}
	else {
		dmaXferCfg1.width = ROM_DMA_WIDTH_1;			/* Width is 1 byte */
	}
	dmaXferCfg1.srcInc = ROM_DMA_ADDRINC_1X;	/* Increment source address by width for each data transfer */
	dmaXferCfg1.dstInc = ROM_DMA_ADDRINC_0X;	/* Increment destination address by width for each data transfer */
	if (ROM_DMA_BuildDescriptorChain(dmaHandle1, &dmaXferCfg1, &dmaSPITXDesc1, NULL) != LPC_OK) {
		errorOut("Error building descriptor chain (SPI TX single link)\r\n");
	}

	/* Queue descriptor for SPI TX transfer */
	ROM_DMA_QueueDescriptor(dmaHandle1, &spiDmaTXQueue1, &dmaSPITXDesc1);

	/* Start queue */
	ROM_DMA_StartQueue(dmaHandle1, &spiDmaTXQueue1);
}
...
void spiRecvSetup1(ROM_SPIM_HANDLE_T spimHandle, ROM_SPIM_XFER_T *pXfer)
{
	...
	dmaXferCfg1.src = (void *) &LPC_SPIM_PORT1->RXDAT;
	dmaXferCfg1.dest  = (void *) rx161;
	...
	dmaXferCfg1.clrTrig = 0;									/* Do not clear trigger after this descriptor completes */
	dmaXferCfg1.fireDescCB = 1;							/* Do not fire descriptor callback on complettion of this descriptor */
	dmaXferCfg1.enabCirc = 0;								/* Not a circular buffer */
	dmaXferCfg1.dmaCh = spiDmaRXQueue1.dmaCh;
	dmaXferCfg1.stallDesc = 0;
	if (flen > 8) {
		dmaXferCfg1.width = ROM_DMA_WIDTH_2;			/* Width is 2 bytes */
	}
	...
	if (ROM_DMA_BuildDescriptorChain(dmaHandle1, &dmaXferCfg1, &dmaSPIRXDesc1, NULL) != LPC_OK) {
		errorOut("Error building descriptor chain (SPI TX single link)\r\n");
	}

	/* Queue descriptor for SPI TX transfer */
	ROM_DMA_QueueDescriptor(dmaHandle1, &spiDmaRXQueue1, &dmaSPIRXDesc1);

	/* Start queue */
	ROM_DMA_StartQueue(dmaHandle1, &spiDmaRXQueue1);
}
...
void RTU_Spi1Dev_Init()
{
	uint32_t memSize, *devMem;
		...
		ROM_DMA_RegisterQueueCallback(dmaHandle1, &spiDmaTXQueue1, ROM_DMA_XFERCOMPLETE_CB, (void *) dmaSPITXTransferComplete);
		ROM_DMA_RegisterQueueCallback(dmaHandle1,
									  &spiDmaTXQueue1,
									  ROM_DMA_XFERDESCCOMPLETE_CB,
									  (void *) dmaSPITXTransferDescComplete);
		ROM_DMA_RegisterQueueCallback(dmaHandle1, &spiDmaTXQueue1, ROM_DMA_XFERERROR_CB, (void *) dmaSPITXTransferError);
	
		/* Register error, descriptor completion, and descriptor chain completion callbacks for SPI RX channel */
		ROM_DMA_RegisterQueueCallback(dmaHandle1, &spiDmaRXQueue1, ROM_DMA_XFERCOMPLETE_CB, (void *) dmaSPIRXTransferComplete1);
		ROM_DMA_RegisterQueueCallback(dmaHandle1,
									  &spiDmaRXQueue1,
									  ROM_DMA_XFERDESCCOMPLETE_CB,
									  (void *) dmaSPIRXTransferDescComplete);
		ROM_DMA_RegisterQueueCallback(dmaHandle1, &spiDmaRXQueue1, ROM_DMA_XFERERROR_CB, (void *) dmaSPIRXTransferError);
		...
		ROM_SPIM_RegisterCallback(spimHandle1, ROM_SPIM_ASSERTSSEL_CB, (void *) CBspiMasterXferCSAssertCB);
		ROM_SPIM_RegisterCallback(spimHandle1, ROM_SPIM_DEASSERTSSEL_CB, (void *) CBspiMMasterXferCSDeAssertCB);
...
uint8_t SPI_Send_One_Data(uint8_t spi_Port,uint8_t tlv5614port,uint32_t data)
{
	...
	{
		//devptr = &dev_spi[1];
		tlv5614head = 0x1 <<12; 	//fast ??,0 slow??
		tlv5614head |= tlv5614port<<14;
		
	}
//	tx16[spiCnt%BUFFSIZE] = (uint16_t)data & tlv5614head; // ? 5614???? A1A0 PWR SPD+12bit data
	tx16[0] = (uint16_t)data | tlv5614head; // ? 5614???? A1A0 PWR SPD+12bit data
	/* Populate some TX data and clear RX data */
	...
	//		mXfer.flags = ROM_SPIM_FLAG_LOOPBACK | ROM_SPIM_FLAG_DMATX | ROM_SPIM_FLAG_DMARX;
			...
			if (mXfer.status != LPC_OK) {
				...
				for (i = 0; i < BUFFSIZE; i++) {
#if XFERSZ > 8
					DEBUGOUT("%04x %04x : ", tx161[i], rx161[i]);
#else
					DEBUGOUT("%02x %02x : ", tx161[i], tx161[i]);
#endif
				}
			}
			spiCnt = dmaCnt = 0;
