...
// IO模拟i2c 时钟线 端口和管脚   
static uint8_t simulation_i2c_sck_port[2]={I2C0_SCK_PORT,I2C1_SCK_PORT};
static uint8_t simulation_i2c_sck_pin[2]={I2C0_SCK_PIN,I2C1_SCK_PIN};

// IO模拟i2c 数据线 端口和管脚   
static uint8_t simulation_i2c_sda_port[2]={I2C0_SDA_PORT,I2C1_SDA_PORT};
static uint8_t simulation_i2c_sda_pin[2]={I2C0_SDA_PIN,I2C1_SDA_PIN};
...
uint8_t IIC_WriteBuffer(uint8_t ID,uint8_t addr, uint8_t reg, uint8_t len, uint8_t * data)
{
    int i;
    IIC_Start(ID);                  	 //起始信号
    IIC_Send_Byte(ID,addr << 1 | I2C_TX_FLAG);
...
uint8_t IIC_ReadBuffer(uint8_t ID,uint8_t addr, uint8_t reg, uint8_t len, uint8_t *buf)
{
    ...
    IIC_RecvACK(ID);
    IIC_Start(ID);                   //起始信号
    IIC_Send_Byte(ID,addr << 1 | I2C_RX_FLAG);
    IIC_RecvACK(ID);
    while (len) {
