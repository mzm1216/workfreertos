					...
					uint8_t DstPeripheral)
{
	...
	/* Enable DMA channels, little endian */
 	pGPDMA->CONFIG = GPDMA_DMACConfig_E|GPDMA_DMACConfig_M;
//	pGPDMA->CONFIG = GPDMA_DMACConfig_E;
						   ...
						   uint32_t Size)
{
	...
	#ifdef TESTMZM_16_2_8
						(uint32_t) GPDMA_WIDTH_HALFWORD,
	#else
						(uint32_t) GPDMA_WIDTH_BYTE,
	#endif
						(uint32_t) GPDMA_WIDTH_BYTE);


//	cwrd = makeCtrlWord(&GPDMACfg,
//						(uint32_t) GPDMA_BSIZE_1,
//						(uint32_t)GPDMA_BSIZE_1,
//						(uint32_t) GPDMA_WIDTH_BYTE,
//						(uint32_t) GPDMA_WIDTH_BYTE);
...
/* Do a DMA scatter-gather transfer M2M, M2P,P2M or P2P using DMA descriptors */
Status Chip_GPDMA_SGTransfer_Not_Start(LPC_GPDMA_T *pGPDMA,
							 uint8_t ChannelNum,
							 const DMA_TransferDescriptor_t *DMADescriptor,
							 GPDMA_FLOW_CONTROL_T TransferType)
{
