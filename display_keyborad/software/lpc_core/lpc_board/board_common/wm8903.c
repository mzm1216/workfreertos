...
#define WM8903_STATE_OFF			0
#define WM8903_STATE_ON 			1

#define WM8903_LOCK_TIMEOUT 		10
...
const Codec_Cfg_t g_CodecCfgs[2] = {
	{ WM8903_FLL_1288MHZ_K, WM8903_FLL_1288MHZ_N}, /*I2S_48000*/
	{ WM8903_FLL_1128MHZ_K, WM8903_FLL_1128MHZ_N}, /*I2S_44100*/
};

#define WM8903_INIT_STEPS	31
const WM8903_Init_Seq_t g_wm8903[] = {
	...
	{ 0x21, WM8903_DAC_DIGITAL_1_VALUE}, // DAC Digital 1(21H): 	 0000  DAC_MONO=0, DAC_SB_FILT=0, DAC_MUTERATE=0, DAC_UNMUTE_RAMP=1, DAC_OSR128=1, DAC_MUTE=0, DEEMPH=00
	...
	{ WM8903_FLL_CONTROL_1, 0x0000}, /* FLL_FRACN_ENA=1, FLL_OSC_ENA=0, FLL_ENA=0 */
#if 1 // Ref clk = 12 MHz MCLK
	{ WM8903_FLL_CONTROL_2, 0x0700}, /* FLL_OUTDIV=00_0111, FLL_CTRL_RATE=000, FLL_FRATIO=000 */
	{ WM8903_FLL_CONTROL_3, WM8903_FLL_1288MHZ_K}, /* FLL_K = 0 */
	{ WM8903_FLL_CONTROL_4, WM8903_FLL_1288MHZ_N}, /* FLL_N = 96 */
	{ WM8903_FLL_CONTROL_5, 0x0000}, /* FLL Clock src = 0 (MCLK) */
#endif

#if 0 // Ref clk = 0.512 MHz Bit Clk
	{ WM8903_FLL_CONTROL_2, 0x0701}, /* FLL_OUTDIV=00_0111, FLL_CTRL_RATE=000, FLL_FRATIO=001 */
	{ WM8903_FLL_CONTROL_3, 0}, /* FLL_K = 0 */
	{ WM8903_FLL_CONTROL_4, 0x600}, /* FLL_N = 96 */
	{ WM8903_FLL_CONTROL_5, 0x0001}, /* FLL Clock src = 0 (MCLK) */
#endif

{ WM8903_FLL_CONTROL_1, 0x0005}, /* FLL_FRACN_ENA=1, FLL_OSC_ENA=0, FLL_ENA=1 */
...
#define WM8903_INPUT_INIT_STEPS 4
const WM8903_Init_Seq_t g_wm8903_input_init[] = {
	/* enable ADC for line-in capture */
	{ 0x0C, 0x0003}, // INL_ENA=1, INR_ENA=1
 ...
 ****************************************************************************/
int WM8903_MultiRegWrite(const WM8903_Init_Seq_t* seq, uint32_t cnt);
...
static int Audio_Codec_SetDefaultValues(const uint8_t *values, int sz)
{
	...
	ret = WM8903_MultiRegWrite((const WM8903_Init_Seq_t *)values, sz);
	rd_val = WM8903_REG_Read(0x7F);
	while ( (rd_val & 0x4) != 0x4 )
	{
		rd_val = WM8903_REG_Read(0x7F);
	}
	WM8903_REG_Write(0x16, 0x4006 | 0x8);
...
uint16_t WM8903_REG_Read(uint8_t reg) {
	uint8_t rx_data[2];
	if (Chip_I2C_MasterCmdRead(CODEC_I2C_BUS, I2CDEV_CODEC_ADDR, reg, rx_data, 2) == 2) {
...
void WM8903_REG_Write(uint8_t reg, uint16_t val)
{
	...
	Chip_I2C_MasterSend(CODEC_I2C_BUS, I2CDEV_CODEC_ADDR, dat, sizeof(dat));
}
...
{
	uint16_t ret;
	WM8903_REG_Write(reg, val);
	ret = WM8903_REG_Read(reg);
	return ret == val;
}
...
{
	int i;
	if (Chip_I2C_MasterCmdRead(CODEC_I2C_BUS, I2CDEV_CODEC_ADDR, reg, buff, len) != len) {
...
{
	I2C_EVENTHANDLER_T old = Chip_I2C_GetMasterEventHandler(CODEC_I2C_BUS);
	int ret;

	/* Initialize I2C */
	Board_I2C_Init(CODEC_I2C_BUS);
	Chip_I2C_Init(CODEC_I2C_BUS);
	Chip_I2C_SetClockRate(CODEC_I2C_BUS, 100000);
	Chip_I2C_SetMasterEventHandler(CODEC_I2C_BUS, Chip_I2C_EventHandlerPolling);

	/* Initialize the default values */
	ret = Audio_Codec_SetDefaultValues((void *)&g_wm8903[0], sizeof(g_wm8903)/sizeof(WM8903_Init_Seq_t));

	Chip_I2C_SetMasterEventHandler(CODEC_I2C_BUS, old);
...
int WM8903_MultiRegWrite(const WM8903_Init_Seq_t* seq, uint32_t cnt)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		if (seq[i].reg_adr == 0xFF) {
			...
			do { 
				rd_val = WM8903_REG_Read(0x70);
			} while (rd_val & 1);
		} else {
			WM8903_REG_Write(seq[i].reg_adr, seq[i].reg_val);
						rd_val = WM8903_REG_Read(seq[i].reg_adr);
		}
	}
	return (SUCCESS);
}
