...
/* 发射灯、告警灯、接收灯*/
static const uint8_t ledBits[3] = {1,8,9};
...
static void Board_LED_Init()
{
	...
	{
		Chip_GPIO_SetPinDIROutput(LPC_GPIO_PORT,LED_PORT, ledBits[i]);
		Chip_GPIO_SetPinState(LPC_GPIO_PORT, LED_PORT, ledBits[i], true);
	}
...
void Board_LED_Set(uint8_t LEDNumber, bool On)
{
	...
	{
		Chip_GPIO_SetPinState(LPC_GPIO_PORT, LED_PORT, ledBits[LEDNumber], On);
	}
}
...
bool Board_LED_Test(uint8_t LEDNumber)
{
	...
	if (LEDNumber < sizeof(ledBits)) {
		return (bool) !Chip_GPIO_GetPinState(LPC_GPIO_PORT, LED_PORT, ledBits[LEDNumber]);
	}
...
//	Board_LED_Set(LEDNumber, !Board_LED_Test(LEDNumber));
	if (LEDNumber < sizeof(ledBits)) {
		Chip_GPIO_SetPinToggle(LPC_GPIO_PORT, LED_PORT, ledBits[LEDNumber]);
	}
}
