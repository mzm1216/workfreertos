...
int ssp_main_init(void)
{
	...
	//
	//NVIC_SetPriority(SSP_IRQ, ((0x01 << 3) | 0x01));
...
void ssp_send_one_byte(uint8_t *dat,uint32_t len)
{	uint32_t tmpret,i;
	//*((uint8_t*)xf_setup.tx_data) = dat;
	memcpy(xf_setup.tx_data,dat,len);
...
//	for(i=0;i<4;i++)
	//	{
	//	tmpret=Chip_SSP_Int_RWFrames8Bits(LPC_SSP, &xf_setup);
	//	if(tmpret==ERROR)
	//	{
	//		continue;//Chip_SSP_RWFrames_Blocking(LPC_SSP, &xf_setup);
	//	}
	//	else{
	//		break;
	//		}
